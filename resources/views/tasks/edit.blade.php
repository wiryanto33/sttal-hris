            ...
            <div class="row">
                <div class="col-12 col-md-6 order-md-1 order-last">
                    ...
                    <p class="text-subtitle text-muted">
                        Edit Tugas yang harus diselesaikan
                    </p>
                </div>
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('tasks.update', $task->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            @enderror">
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}" @if ($task->assigned_to == $user->id) selected @endif
                                        @if (old('assigned_to') == $user->id) selected @endif>{{ $user->userDetail->pangkat }}
                                        {{ $user->userDetail->korps }} {{ $user->name }}</option>
                                @endforeach
                            </select>
                            @error('assigned_to')
                                <div class="invalid-feedback">
                        ...
                        <div class="mb-3">
                            ...
                            @enderror"
                                value="{{ old('file', $task->file) }}" name="file">
                            @error('file')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
